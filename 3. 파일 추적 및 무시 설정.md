...
**중요 주의사항**
 `.gitignore`은 **Git이 아직 추적하지 않은 파일**에만 적용돼.
...
>  이름은 `git`이 요구하는 건 아니며, 그냥 **관례적인 이름일 뿐**이야.
...
- 하지만 커뮤니티에서는 `.gitkeep`을 **암묵적 표준**으로 받아들이고 있어
...
> `.gitignore`에 추가할 경우, 추적을 중지하고 다시 커밋되지 않게 막을 수 있어
...
> 이 명령은 실제 파일을 이동시키고, 동시에 Git에게 **"이건 새 파일이 아니라 이동이야"** 라고 알려줘
...
> Git은 자동으로 rename을 감지하지만, `git mv`를 쓰면 명시적으로 관리 가능하고 히스토리 추적에 유리해
...
- `core.ignorecase`는 Git의 내부 설정으로,
   **파일 이름의 대소문자를 무시할지 여부를 결정**하는 옵션이야
- 기본값:
